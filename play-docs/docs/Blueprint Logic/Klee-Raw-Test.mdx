---
title: Klee Raw Canvas Test
description: Raw MDX canvas test for Klee Blueprint rendering without the PLA4Y BlueprintFlow wrapper.
---

# Klee Raw Canvas Test

This page tests Klee in the simplest possible way inside Docusaurus.

It does not use the PLA4Y `BlueprintFlow` wrapper.

It uses a raw canvas with the `klee` class, which is the same core setup used by Klee.

## Raw Canvas Example

<canvas
  className="klee"
  data-klee-paste="true"
  style={{ width: '100%', height: '600px', background: '#111827', border: '1px solid #4b5563', borderRadius: '18px' }}
>
{`
...
If this page fails, the issue is not the PLA4Y wrapper.

It means Klee is not initializing correctly inside the Docusaurus page, or the script path is still wrong.
